...
#__all__ = ['geometry','plot','utils','interp']
#from . import geometry, plot, utils, interp
from . import geometry, export, interp, test
from .geometry.xy2index import *
from .geometry.find_bc  import *
from .geometry.GetXy    import *
from .geometry.grid     import * #getXyGrid
from .geometry.MFschedule import *
from .geometry.PointsInPolygon import PointsInPolygon

from .plot.axes_default import *

from .utils.RecoverPestIES import *
from .utils.ReadHobOut import *
from .utils.print      import print_
from .utils.varargout  import varargout
from .utils.islinux import islinux
from .elapsed_time     import elapsed_time
from .LpfToUpw         import LpfToUpw
from .utils.makedir import makedir
from .interp.interpPointToGrid import interpPointToGrid
...
# }}}
# def updateDis {{{
def updateDis(mf,nper=[],perlen=[],nstp=[],steady=None,
...
def DrawdownFile(mf): # {{{
    '''
    explain
     import drawdown objective with "mf" field.

    usage
     hobj = flopyUtils.DrawdownFile(mf)
    '''

    return flopy.utils.binaryfile.HeadFile(mf.model_ws+'/'+mf.name+'.ddn',text='drawdown')
# }}}
def UcnFile(mt): # {{{
    '''
    explain
...
def flopyInterpFromPoints(mf,x,y,data,variogram_model='spherical',
    ...
    # }}}
# interpFromPoints {{{
def interpFromPoints(mf,x,y,data,variogram_model='spherical',
        variogram_parameters=None,enable_plotting=False,method='universal'):

   data = flopyInterpFromPoints(mf,x,y,data,
...
def mf96LoadNam(filename=[],debug=False,model_ws='./'):# {{{
    '''
    Explain
    ...
    Inputs
     filename - ".MFI" filename of modflow-96.
     model_ws - working directory
    ...
    # where is filename
    filename = os.path.join(model_ws, filename)
    if not os.path.isfile(filename):
       raise Exception("ERROR: we cannot find %s."%(filename))
...
def mf96LoadBas(filename=[],fileunit=[],debug=False): # {{{
    ...
    with open(filename_bas,'r') as fid: # {{{
        ...
        # 2 heading
        line = fid.readline()
        mfPrint('   3 NLAY NROW NCOL NPER ITMUNI',debug=debug)
        ...
        if debug:
           for idx, _s in enumerate(['undefined','seconds','minutes','hours','days','years']):
              if idx == itmuni:
                 print('   ITMUNI = {}'.format(_s))
...
def mf96LoadBcf(filename=[],fileunit=[],debug=False,nper=[],nlay=[],ncol=[],nrow=[]):# {{{
    ...
    with open(filename_bcf,'r') as fid: # {{{
        ...
        hdry   = float(data[2])
        iwdflg = int(data[3]) # determines if the wetting capability is active
        ...
        if debug: # show brief information.
           print('   ISS(steady state flag)    = {}'.format(iss))
           print('   IBCFCB(unit number)       = {}'.format(ibcfcb))
           print('   HDRY(dry cell)            = {}'.format(hdry))
           print('   IWDFLG(wetting cabaility) = {}'.format(iwdflg))
        ...
        data = fid.readline() # not splitting... Just read one by one...
        ltype  = np.zeros((nlay,),dtype=int)
        laycon = np.zeros((nlay,),dtype=int)
        for l in range(nlay):
           if data[2*l] == ' ':
              ltype[l]  = 0
           else:
              ltype[l]  = int(data[2*l])
           laycon[l] = int(data[2*l+1])
        mfPrint('   ltype  = {}'.format(ltype))
        mfPrint('   laycon = {}'.format(laycon))
        ...
        for nl in range(nlay): # {{{
            mfPrint('Layer = %d'%(nl),debug=debug)
            # 6. Sf1(NCOL, NROW)# {{{
            '''
      Sf1--is the primary storage coefficient. Read only for a transient simulation (steady-state flag, ISS, is 0). For LAYCON equal to 1, Sf1 will always be specific yield, while for LAYCON equal to 2 or 3, Sf1 will always be confined storage coefficient. For LAYCON equal to 0, Sf1 would normally be confined storage coefficient; however, a LAYCON value of 0 can also be used to simulate water-table conditions where drawdowns are expected to remain everywhere a small fraction of the saturated thickness, and where there is no layer above, or flow from above is negligible. In this case, specific yield values would be entered for Sf1.

            * Under confined aquifer -  storage coefficient.
            * Under unconfined aquifer - specific yield value.
            '''
            if iss == 0: # under transient
               cn = 0
               data = fid.readline().split()
               mfPrint('   6. SF1(NCOL, NROW)',debug=debug)
               mfPrint('   data type = {}'.format(data[4:]),debug=debug)
               while True:
            ...
            # 7. Tran(NCOL, NROW) # {{{
            if laycon[nl] == 0 or laycon[nl] == 2:
                cn = 0
                data = fid.readline().split()
                mfPrint('   6. Tran(NCOL, NROW)',debug=debug)
                mfprint('   annotation= {}'.format(data[4:]),debug=debug)
                while True:
                    data = fid.readline().split()
            ...
            # 8. hk(NCOL, NROW) # {{{
            if laycon[nl] == 1 or laycon[nl] == 3:
                cn = 0
                data = fid.readline().split()
                mfPrint('   8. hk(NCOL, NROW)',debug=debug)
                mfPrint('   annotation= {}'.format(data[4:]),debug=debug)
                if data[0] == "0":
            ...
            if laycon[nl] == 1 or laycon[nl] == 3:
                ...
                string=string+'   #{}'.format(''.join(data[4:]))
                mfPrint(string,debug=debug)
                if data[0] == "0":
            ...
            '''
            Explain
             Sf2 - secondary storage coefficient.

            * always specific yield
            '''
            if iss == 0 and  (laycon[nl] == 1 or laycon[nl] == 3):
                cn = 0
                data = fid.readline().split()
            ...
            # 12. Top(NCOL, NROW) # {{{
            if laycon[nl] == 2 or laycon[nl] == 3:
                data = fid.readline().split()
                cn = 0
                string='   12. Top(NCOL, NROW)'
                string=string+'   #{}'.format(''.join(data[4:]))
                mfPrint(string,debug=debug)
                if data[0] == "0":
                    print(data)
            ...
            # 13. wetdry(NCOL, NROW) # {{{
            if iwdflg != 0 and (laycon[nl] == 1 or laycon[nl] == 3):
...
def mf96LoadWel(filename=[],fileunit=[],debug=False,nper=[],nlay=[],ncol=[],nrow=[]):# {{{
    ...
    '''
    mfPrint('Load WEL Package',debug=debug)

    # check file directory
    model_ws, head = os.path.split(filename)
    #mfPrint('model ws = %s'%(model_ws),debug=debug)
    #mfPrint('head     = %s'%(head),debug=debug)

    # get all WEL file from working directory
    lists = glob.glob(os.path.join(model_ws,'*.WEL'))
    for l in lists:
       if l.upper() == os.path.join(model_ws.upper(), head.upper()):
          filename = l
          #mfPrint('find %s.'%(filename),debug=debug)
          break;
...
def mf96LoadDrn(filename=[],fileunit=[],debug=False,nper=[],nlay=[],ncol=[],nrow=[]):# {{{
    ...
    mfPrint('Load DRN Package',debug=debug)
